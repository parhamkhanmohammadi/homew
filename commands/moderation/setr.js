...
module.exports = {
    ...
    name: "setreject",
    aliases: ["setr"],
    description: "Sets A verify Role For Muted Users!",
  ...
  run: async (bot, message, args) => {
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "**Shoma Role Makhsus bara dadn role ro nadrid :)**"
      );
    if (!args[0]) {
      let b = await db.fetch(`reject_${message.guild.id}`);
      let roleName = message.guild.roles.cache.get(b);
      if (message.guild.roles.cache.has(b)) {
        return message.channel.send(
          `**Verify Role is \`${roleName.name}\`!**`
        );
      } else
        return message.channel.send(
    ...
    try {
      let a = await db.fetch(`reject_${message.guild.id}`);
      ...
      if (role.id === a) {
        ...
        db.set(`reject_${message.guild.id}`, role.id);
